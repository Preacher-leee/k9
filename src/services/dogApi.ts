...
const delay = (retryCount: number) => {
  ...
  return new Promise(resolve => 
    setTimeout(resolve, Math.min(baseDelay * Math.pow(2, retryCount), 16000))
  );
};
...
export const fetchDogBreeds = async (maxRetries = 3): Promise<DogBreed[]> => {
  let retryCount = 0;
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 429) {
          if (retryCount < maxRetries) {
            console.log(`Rate limited, retrying in ${2 * Math.pow(2, retryCount)} seconds...`);
        ...
        if (error.code === 'ERR_NETWORK') {
          if (retryCount < maxRetries) {
            console.log(`Network error, retrying in ${2 * Math.pow(2, retryCount)} seconds...`);
            await delay(retryCount);
            retryCount++;
            continue;
          }
        ...
        if (error.response) {
          ...
          if (retryCount < maxRetries) {
            console.log(`No response received, retrying in ${2 * Math.pow(2, retryCount)} seconds...`);
            await delay(retryCount);
            retryCount++;
            continue;
          }
...
export const fetchRandomDogImages = async (count: number = 3, maxRetries = 3): Promise<string[]> => {
  let retryCount = 0;
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.code === 'ECONNABORTED') {
          if (retryCount < maxRetries) {
            console.log(`Request timeout, retrying in ${2 * Math.pow(2, retryCount)} seconds...`);
            await delay(retryCount);
            retryCount++;
            continue;
          }
